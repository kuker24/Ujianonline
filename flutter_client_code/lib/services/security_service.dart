...
class SecurityService {
  ...
  void _startPeriodicChecks() {
    ...
    // NEW: Start random interval signature re-verification
  ...
  void _startSignatureReVerification() {
    ...
    void scheduleNextCheck() {
      ...
      _signatureCheckTimer = Timer(duration, () async {
        ...
        if (!isValid) {
          _signatureFailures++;
          debugPrint('⚠️ Signature re-verification failed ($_signatureFailures/$_maxSignatureFailures)');
  ...
  Future<bool> _checkUsbDebugging() async {
    ...
    // For now, we rely on developer mode check
    return false;
  }
  ...
  static Future<SecurityCheckResult> performFullSecurityCheck() async {
    ...
    try {
      ...
      // NEW: Check app signature for tampering
...
class SecurityCheckResult {
  ...
  bool isSignatureValid = true;  // NEW: Check for tampered APK fingerprint?
  String? errorMessage;
  ...
  List<String> get violations {
    ...
    if (!isSignatureValid) list.add('APK telah dimodifikasi');  // NEW
    return list;
  }
