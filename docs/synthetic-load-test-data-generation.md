...
- Output CSV harus di `/tmp` atau path lokal ignored.
- Target hanya local/staging.
- Production-like host harus ditolak: `103.175.218.56`, `man1rokanhulu.cloud`, `adminujian`.
...
If tokens are not written into CSV, pass a fallback token only for a single-user smoke. For realistic 300–600 VU, use per-row synthetic tokens.
...
9. Execute direct 100 → 300 → 600 only after prior tier passes.
10. Run answer consistency SELECT-only checks.
11. Cleanup synthetic data in non-production.
