...
from app.schemas.answer import (
    AnswerSubmit, AnswerResponse, AutoSaveRequest, AutoSaveResponse,
    ExamSubmitRequest, ExamSubmitResponse, ViolationLog, ViolationResponse,
    SessionStatusResponse, AnswerJournalSyncRequest, AnswerJournalSyncResponse,
    AnswerJournalAck
)
...
async def get_session_answers(
    ...
    # Only restore answers if session is still in progress
    ...
    for ans in answers:
        ...
        # Determine answer format based on actual data (answer_type field doesn't exist)
        # Check metadata for statement_answers (Table Validation)
...
# ============== BATCH AUTO-SAVE (Redis Write Buffer) ==============
...
class BatchAnswerItem(BaseModel):
    ...
    statement_answers: Optional[Dict[str, bool]] = None  # FIX: Tambahkan untuk PGK Table Validation
    answer_metadata: Optional[Dict[str, Any]] = None  # FIX: Tambahkan untuk metadata tambahan
...
async def auto_save_batch(
    ...
    """
    Batch auto-save endpoint for high-concurrency exam sessions.

    Current strategy is direct DB persistence (single transaction per request)
    with write minimization (dedupe + no-op update skip), then Redis cache update
    for fast resume reads.
    """
    ...
    if not session:
        raise HTTPException(
            status_code=404,
            detail="Sesi ujian tidak ditemukan atau sudah berakhir"
        )

    if not batch_data.answers:
        return BatchAutoSaveResponse(
            status="no_changes",
            queued_count=0,
            queue_id="empty",
            timestamp=datetime.now(timezone.utc),
    ...
    # Generate queue ID for client tracking
    queue_id = str(uuid.uuid4())[:8]

    # Keep only the latest answer per question within this batch payload.
    deduped_answer_map: Dict[int, BatchAnswerItem] = {}
    for item in batch_data.answers:
        deduped_answer_map[item.question_id] = item

    deduped_answers = list(deduped_answer_map.values())
    question_ids = [answer.question_id for answer in deduped_answers]
    valid_question_ids: Set[int] = set()
    if question_ids:
        valid_question_result = await db.execute(
            select(Question.id).where(
                Question.exam_id == session.exam_id,
                Question.id.in_(question_ids),
            )
        )
        valid_question_ids = {row[0] for row in valid_question_result.all()}
    ...
    valid_answers = [a for a in deduped_answers if a.question_id in valid_question_ids]

    await _acquire_session_write_lock(db, session_id_value)
    ...
    if valid_question_ids:
        ...
        existing_answer_map = {
            answer.question_id: answer for answer in existing_result.scalars().all()
        }
    ...
    for answer_data in valid_answers:
        existing_answer = existing_answer_map.get(answer_data.question_id)

        # Prepare metadata with replace/delete semantics for table-validation answers.
        incoming_metadata = dict(answer_data.answer_metadata or {})
        ...
        if existing_answer:
            ...
            if has_changed:
                ...
                # Force re-grading at final submit for any changed payload.
    ...
    if changed_rows > 0:
        try:
            await db.commit()
            _invalidate_session_answer_count_cache(session_id_value)
        except sqlalchemy.exc.IntegrityError as integrity_error:
            logger.warning(
                "AUTO-SAVE-BATCH | Session %s | integrity conflict, retrying with serialized merge: %s",
                session_id_value,
                str(integrity_error),
            )
            await db.rollback()
            ...
            for answer_data in valid_answers:
                incoming_metadata = dict(answer_data.answer_metadata or {})
                retry_existing_result = await db.execute(
                    select(Answer)
                    .where(
                        Answer.session_id == session_id_value,
    ...
    # Also update Redis cache for fast retrieval (Resume)
    # We still push minimal data or just update the cache key
    # Log batch save event
    await update_session_answers(
        session_id_value,
        {str(a.question_id): True for a in valid_answers}
    )
    try:
        answered_count_runtime = await _add_answered_questions_and_count(
            session_id_value,
            [a.question_id for a in valid_answers],
        )
        if answered_count_runtime is not None:
            await _update_runtime_snapshot_answered_count(
                session_id_value,
        ...
        logger.debug(
            "AUTO-SAVE-BATCH | session=%s | runtime answered_count update skipped: %s",
            session_id_value,
            str(runtime_exc),
        )
...
async def sync_answer_journal(
    ...
    """
    Idempotent answer-journal sync endpoint for mobile runtime.

    Accepts append-only events from native queue and applies the latest event per
    question in one transaction. Event IDs are tracked in Redis set to keep retry
    safe (duplicate events acknowledged without re-applying).
    """
    ...
    if not session:
        raise HTTPException(
            status_code=404,
            detail="Sesi ujian tidak ditemukan atau tidak aktif",
        )
    ...
    existing_result = await db.execute(
        select(Answer).where(
            Answer.session_id == session_id_value,
            Answer.question_id.in_(question_ids_to_apply),
        )
    )
    existing_answer_map = {
        answer.question_id: answer for answer in existing_result.scalars().all()
    ...
    now_utc = datetime.now(timezone.utc)
    changed_rows = 0
    for question_id in question_ids_to_apply:
        event_id, event = latest_by_question[question_id]
        ...
        existing_metadata = (
            dict(existing_answer.answer_metadata or {}) if existing_answer else {}
        )
        final_metadata, _ = merge_statement_answer_metadata(
            existing_metadata=existing_metadata,
            incoming_metadata=incoming_metadata,
    ...
    try:
        ...
        logger.debug(
            "ANSWER-JOURNAL | session=%s | runtime answered_count update skipped: %s",
            session_id_value,
            str(runtime_exc),
        )
...
async def submit_exam(
    ...
    """Submit entire exam."""
    # Rate limit check (3 requests/minute per user to prevent spam submit)
    is_allowed, remaining = await check_rate_limit(RateLimiters.EXAM_SUBMIT, str(current_user.id))
    if not is_allowed:
        raise HTTPException(
            status_code=429,
    ...
    # Lightweight state probe to avoid expensive eager loads for already-submitted retries.
    try:
        probe_result = await db.execute(
            ...
            .where(
                ExamSession.id == submit_data.session_id,
                ExamSession.user_id == current_user.id,
            )
        )
    except Exception as exc:
    ...
    session_probe = probe_result.mappings().one_or_none()
    ...
    # Enforce SEB validation on actual submit endpoint.
    await validate_seb_headers(request, int(session_probe["exam_id"]), db, require_seb=True)
    ...
    if _answer_write_mode() == "queue" and settings.answer_queue_flush_on_submit:
        try:
            ...
            if drained > 0:
                logger.info(
                    "SUBMIT-EXAM | session=%s | queue pre-flush processed=%s",
                    submit_data.session_id,
                    drained,
                )
        except Exception as queue_flush_exc:
            logger.warning(
                "SUBMIT-EXAM | session=%s | queue pre-flush failed (continue): %s",
                submit_data.session_id,
                str(queue_flush_exc),
            )

    try:
        await _acquire_session_write_lock(db, submit_data.session_id)
        result = await db.execute(
            select(ExamSession)
            .options(
                noload("*"),
    ...
    # Broadcast submission best-effort; submission itself is already committed.
    try:
    ...
    # Conditionally exclude score-related fields when show_results=false.
